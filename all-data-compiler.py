...
# proteins = ["1BB1", "1BYZ", "1C3T", "1CQ0", "1D7T", "1DJF", "1DN3", "1DNG", "1E0M", "1EC5", "1FME", "1G6U", "1HQJ", "1IC9", "1ICL", "1ICO", "1IHQ", "1J4M", "1JM0", "1JMB", "1JY4", "1JY6", "1JY9", "1K09", "1K43", "1KD8", "1KD9", "1KDD", "1KVF", "1KVG", "1KYC", "1L2Y", "1L4X", "1LB7", "1LE0", "1LE1", "1LQ7", "1LT1", "1M02", "1M3W", "1MFT", "1MJ0", "1MV4", "1N09", "1N0A", "1N0C", "1N0D", "1NA0", "1NA3", "1OVR", "1OVU", "1OVV", "1P68"]
dict_of_pdb = {}
dict_of_superfam = {}
protein_list = []
...
def get_info(superfam):
    res = requests.get('http://www.cathdb.info/version/v4_1_0/api/rest/superfamily/' + superfam, headers={"USER-AGENT":u_a})
    webpage_dict = res.json()
    if webpage_dict["success"] == False:
        print("This superfam doesn't exist")
    if webpage_dict["success"] == True:
        superfam_info = {
        'cath_id' : superfam,
        'child_count_s100' : webpage_dict['data']['child_count_s100_code'],
        'child_count_s35' : webpage_dict['data']['child_count_s35_code'],
        'child_count_s60' : webpage_dict['data']['child_count_s60_code'],
        'child_count_s95' : webpage_dict['data']['child_count_s95_code'],
        'classification_name' : webpage_dict['data']['classification_name'],
        'example_domain_id' : webpage_dict['data']['example_domain_id']
        }
        print("Getting info about this superfamily")
        return superfam_info
...
pdb_superfam = open('/home/ilya/Projects/De-Novo/List-of-De-Novo.tsv', 'w')
pdb_superfam.write("PDB_ID\tSuperfamilies\n")
superfam_info = open('/home/ilya/Projects/De-Novo/superfam_info.tsv', 'w')
superfam_info.write("Superfamily\tClassification Name\tExample Domain\tS100\tS95\tS60\tS35\t\n")



for key in dict_of_pdb: #writes file for pdb
    pdb_superfam.write(key + "\t" + dict_of_pdb[key] + "\n")
pdb_superfam.close

for key in dict_of_superfam:
    try:
        superfam_info.write(dict_of_superfam[key]['cath_id'] + "\t")
    except:
        superfam_info.write("None available\t")
    try:
        superfam_info.write(dict_of_superfam[key]['classification_name'] + "\t")
    except:
        superfam_info.write("None available\t")
    try:
        superfam_info.write(dict_of_superfam[key]['example_domain_id'] + "\t")
    except:
        superfam_info.write("None available\t")
    try:
        superfam_info.write(dict_of_superfam[key]['child_count_s100'] + "\t")
    except:
        superfam_info.write("None available\t")
    try:
        superfam_info.write(dict_of_superfam[key]['child_count_s95'] + "\t")
    except:
        superfam_info.write("None available\t")
    try:
        superfam_info.write(dict_of_superfam[key]['child_count_s60'] + "\n")
    except:
        superfam_info.write("None available\n")
superfam_info.close()
